...
const git = {
     ...
     */
    getTags() {
        return exec(`git tag -l`).trim().split('\n');
    },
    ...
    /**
     * return a list of git commits on current branch.
     * @return {Array} an array of commit objects
     */
    getAllCommits() {
        const rawCommits = exec(`git log --pretty=oneline`).trim().split('\n');
    ...
    getDeltaCommits(v1, v2) {
        ...
        if (semver.valid(v1) === null || semver.valid(v2) === null) {
            log.fatal(`Invalid semver: ${v1}, ${v2}`);
        }

        const versions = this.getVersions();

        if (!_.includes(versions, v1)) {
            log.fail(`${v1} does not exist!`);
        }

        if (!_.includes(versions, v2)) {
            log.fail(`${v2} does not exist!`);
        }
    ...
    tag(name, annotation, dryRun) {
        ...
        if (_.includes(tags, realTagName)) {
            ...
            cmd =
                `git tag ${realTagName} --cleanup=whitespace -f -m "Release\n` +
                escAnnotation +
                '"';
        }
    ...
    parseConventionalCommit(msg) {
        ...
        if (results.length !== 4) {
            ...
            return {
                ...
                // it might be a "release commit" like ones generated by npm
                // commands, where the commit message is just a semver (1.0.0).
                // if so, use "release" instead of "unknown"
                type: semver.valid(results[2]) !== null ? 'release' : 'unknown',
                typeDetail: null
            };
        }
